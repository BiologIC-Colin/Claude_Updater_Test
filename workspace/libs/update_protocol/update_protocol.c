...
int update_protocol_encode_start(uint8_t *buffer, size_t buf_len, uint32_t image_size)
{
	...
	buffer[0] = 0x01; /* START message type */
	buffer[1] = image_size & 0xff;
	buffer[2] = (image_size >> 8) & 0xff;
	buffer[3] = (image_size >> 16) & 0xff;
	buffer[4] = (image_size >> 24) & 0xff;
                                 ...
                                 size_t data_len)
{
	...
	if (data_len > UPDATE_PROTOCOL_MAX_PAYLOAD) {
		return -EINVAL;
	}

	buffer[0] = 0x02; /* DATA message type */
	buffer[1] = sequence & 0xff;
...
int update_protocol_encode_end(uint8_t *buffer, size_t buf_len, uint32_t crc32)
{
	...
	buffer[0] = 0x03; /* END message type */
	buffer[1] = crc32 & 0xff;
	buffer[2] = (crc32 >> 8) & 0xff;
	buffer[3] = (crc32 >> 16) & 0xff;
	buffer[4] = (crc32 >> 24) & 0xff;
